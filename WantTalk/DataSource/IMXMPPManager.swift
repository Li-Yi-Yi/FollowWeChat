...
class IMXMPPManager: NSObject, XMPPStreamDelegate, XMPPRosterDelegate,/*UIAlertViewDelegate,*/XMPPReconnectDelegate, XMPPRoomDelegate, XMPPMUCDelegate {
    ...
    func xmppStreamConnectDidTimeout(_ sender: XMPPStream) -> Void {
        ...
        if !self.sessionManager.loggedIn {
            self.streamConnectDelegate?.streamConnectChanged(IMConnectionStatus.timeOut)//超时之后需要做处理
        }
    }
    ...
    func xmppStream(_ sender: XMPPStream, didNotAuthenticate error: XMLElement) -> Void {
        ...
        if self.failHandler != nil {
            self.failHandler!("用户名或密码错误")//不通过也要做处理
        }
    }
    ...
    func converToDate(dateStr:String?=nil)->String?{
        if dateStr == nil {
            return ""
        }
        ...
        var date = format.date(from: dateStr!)
        
        if date == nil{
            let format = DateFormatter()
            format.dateFormat = "yyyy-MM-dd HH:mm:ss"
            date = format.date(from: dateStr!)
        }
            ...
            if let subJect: String = message.elements(forName: "subject").first?.stringValue {
                ...
                if let body = body{
                    if let msg: String = body.components(separatedBy: "_MSG").first {
                        if let from: String = message.attribute(forName: "from")?.stringValue {
                            let keyForDate = self.converToDate(dateStr: body.components(separatedBy: "_MSG").last)
                            var modelContext = self.isNeedEncrypt() ? AESHelper.aes128DecryptXmpp(msg, withKey: keyForDate) : msg//是否需要解密的文字
                            if modelContext == ""{
                                modelContext = msg
                            }
                            chatModel = IMChat(sendId: from.components(separatedBy: "/").first, receivedId: IMSessionManager.sharedSessionManager.currentUserProfile!.jidId, messageContext: modelContext)
    ...
    //判断需要解密的时间
    func isNeedEncrypt() -> Bool{
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd HH:mm:ss:SSS"
        let date = formatter.date(from: "2017-09-28 00:00:00:000")
        
        let date1: Date = Date()
        let result:ComparisonResult = date1.compare(date!)
        print("result = \(result == ComparisonResult.orderedDescending)")
            ...
            if let subJect: String = message.elements(forName: "subject").first?.stringValue {
                ...
                if let msg: String = body.components(separatedBy: "_MSG").first {
                    ...
                    if let from: String = message.attribute(forName: "from")?.stringValue {
                        let keyForDate = self.converToDate(dateStr: body.components(separatedBy: "_MSG").last)
                        var modelContext = ""
                        modelContext = self.isNeedEncrypt() ? (AESHelper.aes128DecryptXmpp(msg, withKey: keyForDate) ?? msg) : msg//是否需要解密的文字
                        if modelContext == ""{
                            modelContext = msg
                        ...
                        if senderName == self.userChineseName!+"(\(IMSessionManager.sharedSessionManager.username ?? ""))" {
                            // 自己发言
                            chatModel = IMChat(sendId: myJid, receivedId: occupantJid, messageContext: modelContext)
                            chatModel.nickName? = senderName
                            isOwn = true
                            return
                        } else {
                            // 對方發言
                            chatModel = IMChat(sendId: occupantJid, receivedId: myJid, messageContext: modelContext)
                            chatModel.nickName? = senderName
                            if modelContext.hasPrefix("【"){                                //加入群更新列表
    ...
    */
    ...
    func getCurrentDateForKey() -> String { //加密key时间2017091415:14:30
        let date: Date = Date()
        let dateFormatter: DateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyyMMddHH:mm:ss"
        print("dateFormatter.dateFormat: \(dateFormatter.string(from: date))")
        return dateFormatter.string(from: date)
    }
    ...
    func sendMessage(chatModel: IMChat) {
        ...
        body.stringValue = AESHelper.aes128EncryptXmpp(chatModel.messageContext!, withKey: self.getCurrentDateForKey()) + "_MSG" + self.getCurrentDate()
    ...
    func sendRoomMessage(chatModel: IMChat) {
        ...
        let msg = XMPPMessage(type: "groupchat", to: reciver)
        msg?.addBody(AESHelper.aes128EncryptXmpp(chatModel.messageContext!, withKey: self.getCurrentDateForKey()) + "_MSG" + self.getCurrentDate())
